...
export default ({
  show, handleClose, task, name, finalSubmit,
}) => {
  const [score, setScore] = useState(task.score ? task.score : 0);
  ...
  async function submitFinal(state) {
    // if(tusk && tusk.studentState === 'DRAFT'){
    //   await reviewRequests.deleteDraft(tusk.id)
    // }
    const obj = {
      id: `req-id-${Date.now()}`,
  ...
  return (
      <>
                    ...
                    id, minScore, maxScore, category, title, description,
                      ...
                      <p>{description}</p>
                            <Range
                                ...
                                idk={id}
                                grade={task.grade ? task.grade[id].score : 0}
        ...
        <Modal show={showFinal} onHide={closeFinal} size={'xl'}>
          ...
          <Modal.Footer>
            <Button variant="secondary" onClick={() => submitFinal('DRAFT')}>
              Сохранить
            </Button>
            <Button variant="primary" onClick={() => submitFinal('PUBLISHED')}>
              Отправить результат
            </Button>
          </Modal.Footer>
        </Modal>
